...
var Store = (function () {
    ...
    if (typeof(Storage) !== 'undefined') {
        ...
        methods.set = function (key, data) {
            // check if it is json - start with '{' - stringify if so, otherwise leave as as and return
            var typedData = (JSON.stringify(data).charAt(0) === '{') ? JSON.stringify(data) :  data;
    ...
    // Cookie Storage
        ...
        methods.set = function (key, data) {                  
            if(!key || !data) { return }
            // check if it is json - start with '{' - stringify if so, otherwise leave as as and return       
            var cData = JSON.stringify(data).charAt(0) ==='{' ? JSON.stringify(data) : data;
            // set cookie
            doc.cookie = encodeURIComponent(key) + '=' + encodeURIComponent(cData) + '; path=/; samesite=lax' // no expiration date - session only
        }

        methods.remove = function (key) {            
            if (!key) { return }
            doc.cookie = escape(key) + '=; expires=Thu, 01 Jan 1970 00:00:00 GMT; path=/; samesite=lax'
        }
