...
class FallBackLanguageMiddleware(LocaleMiddleware):
    ...
    def default_lang(self):
        return settings.LANGUAGE_CODE if hasattr(settings, 'LANGUAGE_CODE') else None

    @property
    def fallback_languages(self):
        return settings.LANGUAGE_CODE_FALLBACK if hasattr(settings, 'LANGUAGE_CODE_FALLBACK') else None

    @property
    def fallback_to_default(self):
        return settings.LANGUAGE_DEFAULT_FALLBACK if hasattr(settings, 'LANGUAGE_DEFAULT_FALLBACK') else None
        ...
        if self.fall_back_dict:
            ...
            if len(request_path) > 0:
                ...
                if possible_language in self.fall_back_dict:
                    redirect_language = self.fall_back_dict.get(possible_language)
