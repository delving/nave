...
def parse_mime_type(mime_type):
    ...
    if full_type == '*':
        full_type = '*/*'
    (main_type, subtype) = full_type.split("/")
    return main_type.strip(), subtype.strip(), params


def parse_media_range(media_range):
    """Carves up a media media_range and returns a tuple of the
       (type, subtype, params) where 'params' is a dictionary
       of all the parameters for the media media_range.
       For example, the media media_range 'application/*;q=0.5' would
       get parsed into:
    ...
    if 'q' not in params or not params['q'] or not float(params['q']) or float(params['q']) > 1 or float(
            params['q']) < 0:
        params['q'] = '1'
...
def fitness_and_quality_parsed(mime_type, parsed_ranges):
    ...
    for (main_type, subtype, params) in parsed_ranges:
        if (main_type == target_type or main_type == '*' or target_type == '*') and \
