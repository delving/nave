...
def test__rdfrecord__ip_filter(rf, settings):
    settings.IP_SPEC_WHITE_LIST = { 'niw-fotoarchief': [ "127.0.0.1" ]}
    request = rf.get('/search/', follow=True)
    request_ip = RDFRecord.get_client_ip(request)
    test = RDFRecord.get_filters_by_ip(request)
    assert request_ip == "127.0.0.1"
    assert len(test) == 0

    # now test without the filter
    request = rf.get('/search/', follow=True)
    request.META['HTTP_X_FORWARDED_FOR'] = "127.0.0.2"
    request_ip = RDFRecord.get_client_ip(request)
    assert request_ip == "127.0.0.2"
    test = RDFRecord.get_filters_by_ip(request)
    assert len(test) == 1
